...
struct ThreadPayload {
    ...
    ThreadPayload(Callable &&fn, Args &&...args) : callable_(fn), args_(args...) {
    }
...
void *pthread_fn(void *payload) { // this takes ownership of the payload
    auto *thread_payload = static_cast<ThreadPayload<Callable, Args...> *>(payload);
...
class Thread {
  public:
    ...
    Thread(Callable &&thread_fn, const Settings &settings = {}, Args... args) {
        ...
        auto *payload =
            new ThreadPayload<Callable, Args...>(std::decay_t<Callable>(thread_fn), std::decay_t<Args>(args)...);

        int res =
            pthread_create(&thread_, nullptr, &pthread_fn<std::decay_t<Callable>, std::decay_t<Args>...>, payload);
